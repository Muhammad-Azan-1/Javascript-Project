let mainDiv = document.querySelector(".container") as HTMLDivElement;
let btn = document.querySelector(".btn") as HTMLButtonElement;
let btn2 = document.querySelector("#btn2") as HTMLButtonElement;
let input = document.querySelector("input") as HTMLInputElement;
let ul = document.querySelector(".list-container") as HTMLUListElement;
let p = document.querySelector("p") as HTMLParagraphElement;
let todo_div = document.querySelector("todo") as HTMLDivElement;

type Custom = { item: string; id: string };

let curentEditinLi: HTMLElement | null = null;
let crossBtn: HTMLSpanElement | null = null;
...
function show(n: number, val: string) {
  if (val == "empty") {
...
//! for add todo
function addTodo() {
 let uniqueId: string = crypto.randomUUID();
  if (input.value.trim() !== "") {

  const todo_item : Custom = ({ item: input.value, id: uniqueId });
    todo.push(todo_item)
...
function createTodoItem(todos:Custom){
  console.log("todo from UI function" , todos)
  
   let li: HTMLElement = document.createElement("li");
  let crossIcon: HTMLSpanElement = document.createElement("span");
  let editIcon: HTMLSpanElement = document.createElement("span");
   ...
   editIcon.setAttribute("class", "span2");
    ...
    li.addEventListener("click", (e) => {
      if (e.target == li) {
        ...
        let new_array = todo.filter(
          (obj: Custom) => li.getAttribute("id") != obj.id
        ); // filter out all items which satisfy the condition and return all them as a new array 
...
btn2.addEventListener("click", (e) => {
  if (input.value.trim() != "") {
    ...
    let new_array = todo.forEach((obj: Custom) => {
