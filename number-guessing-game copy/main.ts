...
let submitField = document.querySelector(".submitfield") as HTMLInputElement;
let guessRecord = document.querySelector(".guessRecord") as HTMLDivElement;
let resultBox = document.querySelector(".resultBox");
let restartBtn = document.querySelector(".restartGame-btn") as HTMLInputElement;
let prevGuess = document.querySelector(".prevGuess") as HTMLSpanElement;
let remaningGuess = document.querySelector(".remaningGuess") as HTMLSpanElement;

let allPrevGuesses: number[] = [];
let noOfGuesses: number = 0;
let remaninGuesses : number;
let message : string ;
...
if (playGame) {
  submitField.addEventListener("click", (e) => {
    e.preventDefault();
    let userInput = document.querySelector(".guessfield") as HTMLInputElement;
    console.log(userInput);
    let userValue: string = userInput.value;
    if (userValue.trim() != "") {
...
function matchGuess(guessNo: number) {
  if (guessNo < random) {
    resultBox?.classList.remove("hide");
