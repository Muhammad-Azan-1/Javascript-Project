...
const pTag = document.querySelector(".box3 p");
...
function CurrenciesOptions() {
    for (let select of selects) {
        for (let currCode in countryList) {
            ...
            if ((select.id == "from" && option.value == "USD") ||
...
CurrenciesOptions();
function changeFlag(currOption) {
    let currCurrency = currOption.value;
...
function changeCurrency() {
    ...
    console.log(value);
    if ((value).trim() == '')
        showError('Please enter a amount first');
    else if (+value <= 0)
        showError('Ammount should be a positive number and must be greater then 0');
    if (latestRate) {
        let fromCurrency = document.querySelector("#from").value.toLocaleLowerCase();
        let toCurrency = document.querySelector("#to").value.toLocaleLowerCase();
