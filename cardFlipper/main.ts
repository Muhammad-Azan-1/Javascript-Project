...
let cardsArray: { name: string; img: string }[] = [
  {
    name: "CSS",
    img: "./assests/css.png",
  },
...
let duplicateArray: Data[] = cardsArray.concat(cardsArray);

// 2 shufling array randomly

function shullfeArray(array: Data[]): Data[] {
...
let shuflledArr: any = shullfeArray(duplicateArray); // original duplicateArray is also changed

// 3 creating card for each value of array


shuflledArr.forEach((items: { name: string; img: string }) => {
...
cardBox?.addEventListener("click", (e) => {
  ...
  let parentEl = targetEl.parentNode as HTMLDivElement;
